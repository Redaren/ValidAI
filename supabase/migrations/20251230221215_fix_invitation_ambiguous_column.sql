-- Migration: Fix Ambiguous Column Reference in handle_existing_user_invitation
-- Purpose: Rename return table columns to avoid ambiguity with table columns
-- Created: 2025-12-30
--
-- PROBLEM:
-- Error 42702: column reference "organization_id" is ambiguous
-- The RETURNS TABLE has a column named "organization_id" which conflicts with
-- the organization_members.organization_id in the ON CONFLICT clause.
--
-- FIX:
-- Rename return table columns to use "result_" prefix to avoid any ambiguity.
...
Updated 2025-12-30:
- Made organization_members INSERT idempotent
- Renamed return columns to result_* prefix to avoid ambiguity with table columns';

-- Re-grant permissions (since we dropped and recreated the function)
