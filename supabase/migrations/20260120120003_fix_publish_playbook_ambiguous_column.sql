-- ============================================================================
-- Migration: Fix publish_playbook ambiguous column reference
-- Purpose: Remove aliases in RETURN QUERY that conflict with RETURNS TABLE columns
-- Bug: "column reference version_number is ambiguous" error
-- Fix: Remove AS aliases since RETURNS TABLE columns map by position
-- ============================================================================
...
BEGIN
  ...
  SELECT *
  INTO v_processor
  FROM validai_processors vp
  WHERE vp.id = p_processor_id
    AND vp.organization_id = v_org_id
  ...
  -- Return result (no aliases needed - maps by position to RETURNS TABLE columns)
  RETURN QUERY SELECT
    v_snapshot_id,
    v_next_version,
    v_operation_count,
    format('Published version %s with %s operations', v_next_version, v_operation_count);
END;
$$;

COMMENT ON FUNCTION publish_playbook(uuid, text) IS
'Creates a frozen snapshot of a processor and its operations. Fixed: Removed ambiguous column aliases in RETURN QUERY.';
